...
def bill(bot: "VkBot", data: str = None):
    withdraw_bill(bot)
    bot.api.send_chat_msg(GUILD_CHAT_ID, f"Списал налог с баланса, проверять можно командой баланс")

    month = (datetime.date.today().month + 12) % 12 + 1
    next_run = datetime.datetime.now().replace(day=15, month=month, hour=0, minute=0, second=0, tzinfo=None)
...
def bill2(bot: "VkBot", data: str = None):
    ...
    month = (datetime.date.today().month + 12) % 12 + 1
    next_run = datetime.datetime.now().replace(day=1, month=month, hour=0, minute=0, second=0, tzinfo=None)
