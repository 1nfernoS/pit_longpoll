...
def user_message(self: VkBot, event: VkBotEvent):
    ...
    if 'https://vip3.activeusers.ru/app.php?' in event.message.text:
        ...
        class_name: ORM.Item = DB.query(ORM.Item).filter(ORM.Item.item_id == class_id).first()
        build = get_books(inv)
        ...
        info: ORM.UserInfo = DB.query(ORM.UserInfo).filter(ORM.UserInfo.user_id == event.message.from_id).first()

        if info:
            info.user_profile_key = auth

            info.user_items = [DB.query(ORM.Item).filter(ORM.Item.item_id == i).first()
                               for i in build]
