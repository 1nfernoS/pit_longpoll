...
def travel_check(self: VkBot, event: VkBotEvent):
    ...
    _danger_list = ('Боль разрывает Вас на части…', 'В воздухе витает отчетливый запах смерти…',
                    'Воздух просто гудит от опасности…', 'Вы уже на пределе…', 'Еще немного, и Вы падаете без сил…',
                    'Кажется, конец близок…', 'Крик отчаяния вырывается у Вас из груди…',
                    'Ноги дрожат от предчувствия беды…', 'Нужно бежать отсюда!', 'Силы быстро Вас покидают…',
                    'Смерть таится за каждым поворотом…', 'Чувство тревоги бьет в колокол!')
