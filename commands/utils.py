...
class SetLeader(Command):
    def __init__(self):
        ...
        self.desc = 'Назначить лидера. Только для создателю'
        self.require_utils = True
        # self.set_active(False)
        return
    ...
    def run(self, bot: VkBot, event: VkBotEvent):
        ...
        if 'reply_message' in event.message.keys():
            ...
            leader_role = 1
            guild_member_role = 5
            if user_to.role_id == leader_role:
                user_to.role_id = guild_member_role
                msg = f"Теперь @id{user_to.user_id} имеет права guild_member"
            else:
                user_to.role_id = leader_role
                msg = f"Теперь @id{user_to.user_id} имеет права leader"
...
class SetOfficer(Command):
    ...
    def run(self, bot: VkBot, event: VkBotEvent):
        ...
        if 'reply_message' in event.message.keys():
            ...
            officer_role = 2
            guild_member_role = 5
            if user_to.role_id == officer_role:
                user_to.role_id = guild_member_role
                msg = f"Теперь @id{user_to.user_id} имеет права guild_member"
            else:
                user_to.role_id = officer_role
                msg = f"Теперь @id{user_to.user_id} имеет права officer"
